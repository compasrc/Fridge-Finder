...
function getIngredientEmoji(ingredient) {
    const mapping = {
        "bread":"🍞","pasta":"🍝","cheese":"🧀","milk":"🥛","nuts":"🌰","eggs":"🥚","butter":"🧈",
        "avocado":"🥑","tomato":"🍅","banana":"🍌","strawberry":"🍓","lettuce":"🥬","jelly":"🍇",
        "naan":"🥯","soy sauce":"🥢","olive oil":"🫒","salt":"🧂","tomato sauce":"🍅","chicken":"🍗",
        "beef":"🥩","pork":"🍖","tuna":"🐟","rice":"🍚","peanut":"🥜"
    };
...
function createIngredientBoxes(){
    ...
    ingredients.forEach(ing=>{
        ...
        box.textContent=`${emoji ? emoji+" " : ""}${ing}`;
...
function getSelectedIngredients(){
    return Array.from(document.querySelectorAll('#ingredients-container .ingredient-box.selected')).trim.toLowerCase());
}
...
function createRecipeCard(recipe, isFavoriteView=false){
    ...
    if (currentUser && card.querySelector('[data-action="favorite"]')) {
        card.querySelector('[data-action="favorite"]').addEventListener('click', () => toggleFavorite(recipe.name));
    }
    if (isFavoriteView && card.querySelector('[data-action="remove-favorite"]')) {
